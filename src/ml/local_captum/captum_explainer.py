    ...
    * :class:`captum.attr.GuidedBackprop`
    ...
    def __init__(
        ...
        **kwargs,
        ...
        import captum.attr
        from .integrated_gradients import IntegratedGradients, custom_compute_gradients
        ...
        self._custom_compute_gradients = custom_compute_gradients
    ...
    def _is_supported_attribution_method(self) -> bool:
        ...
        # Methods that REQUIRE a baselines kwarg with no default are
        # explicitly allowed here when the caller is expected to provide one
        # (IntegratedGradients, GradientShap). Other methods that lack a
        # default baseline argument are rejected.
        REQUIRES_BASELINE_ALLOWED = {'IntegratedGradients', 'GradientShap'}
        name = self.attribution_method_class.__name__
        if self._needs_baseline() and name not in REQUIRES_BASELINE_ALLOWED:
            return False
        return name in self.SUPPORTED_METHODS
    ...
    def forward(
        ...
        **kwargs,
        ...
        # GradientShap propagates `self.gradient_func` to the InputBaselineXGradient
        # it builds internally (captum/attr/_core/gradient_shap.py L267). The hetero
        # input tuple contains edge-mask tensors that the model does not differentiate
        # against, so autograd.grad raises unless allow_unused=True is set. The local
        # IG override already does this via custom_compute_gradients; mirror it on
        # GradientShap here so EG with native Captum noise works on hetero inputs.
        if self.attribution_method_class.__name__ == 'GradientShap':
            self.attribution_method_instance.gradient_func = self._custom_compute_gradients
